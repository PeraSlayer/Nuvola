...
export class Camera {
  ...
  unproject(sx, sy, zRef) {
    ...
    const zx = (sx - this.panX) / this.zoom;
    const zy = (sy - this.panY) / this.zoom;

    const rx = zx + ry;
    const ry2 = zy + lz;

    const lx = (rx * c + ry2 * s) * 0.5;
    const ly = (rx * s - ry2 * c) * 0.5;

    const lz = zRef;

    const x2 = lx * cosY - lz * sinY;
    const z2 = lx * sinY + lz * cosY;
    const lx2 = x2;
    const lz2 = z2;

    const ly2 = ly * cosX + lz2 * sinX;
    const lz3 = -ly * sinX + lz2 * cosX;
