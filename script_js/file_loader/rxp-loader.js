...
let _chunks = [], _totalSize = 0;

self.onmessage = function(e) {
  const msg = e.data;
  if (msg.type === 'init') {
    _totalSize = msg.totalSize;
    _chunks = [];
  } else if (msg.type === 'chunk') {
    _chunks.push(msg.data);
  } else if (msg.type === 'done') {
    try {
      const combined = new Uint8Array(_totalSize);
      let off = 0;
      for (const c of _chunks) { combined.set(new Uint8Array(c), off); off += c.byteLength; }
      _chunks = null;
      const result = parseRXP(combined.buffer);
...
/** Loads Riegl RXP binary point clouds via a Web Worker, with chunked streaming. */
  ...
  load(input) {
    ...
    if (input instanceof ArrayBuffer) {
      ...
      return new Promise((resolve, reject) => {
        ...
        worker.postMessage({ type: 'init', totalSize: input.byteLength });
        worker.postMessage({ type: 'chunk', data: input }, [input]);
    ...
    return new Promise((resolve, reject) => {
      ...
      worker.onmessage = (e) => { worker.terminate(); if (e.data.ok) resolve(e.data); else reject(new Error(e.data.error)); };
      worker.onerror = (err) => { worker.terminate(); reject(err); };
      worker.postMessage({ type: 'init', totalSize: input.size });

      (async () => {
        try {
          let offset = 0;
          while (offset < input.size) {
            const end = Math.min(offset + CHUNK, input.size);
            const chunk = await input.slice(offset, end).arrayBuffer();
            worker.postMessage({ type: 'chunk', data: chunk }, [chunk]);
            offset = end;
          }
          worker.postMessage({ type: 'done' });
        } catch (err) {
